...
RegisterCommand("plist", function(source, args)
    ...
    if xPlayer.PlayerData.job.name == "police" then
        local type = "toggle"
        TriggerEvent("qb-activeofficers:officers:refresh")

        if args[1] == "0" then
            type = "drag"
        end

        TriggerClientEvent("qb-activeofficers:officers:open", src, type)
    end
end)
...
RegisterCommand("callsign", function(source, args)
    ...
    if xPlayer and (xPlayer.PlayerData.job.name == 'police') and args[1] then
        if args[1] == 'none' then
            ...
            TriggerEvent("qb-activeofficers:officers:refresh")
            TriggerClientEvent('QBCore:Notify', source, "استعادة الاسم", "success")
        else
            CallSigns[xPlayer.PlayerData.license] = args[1]
            TriggerEvent("qb-activeofficers:officers:refresh")
            SaveResourceFile(GetCurrentResourceName(), "callsigns.json", json.encode(CallSigns))
            TriggerEvent("qb-activeofficers:officers:refresh")
            TriggerClientEvent('QBCore:Notify', source, "" .. args[1].." تم التحديث الى:", "success")
        end
    end
end)

RegisterServerEvent("qb-activeofficers:officers:refresh")
AddEventHandler("qb-activeofficers:officers:refresh", function()
    local new = ""
    ...
    HTML = new
    TriggerClientEvent("qb-activeofficers:officers:refresh", -1, HTML)
end)
